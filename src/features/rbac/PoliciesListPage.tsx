...
export default function PoliciesPage() {
    ...
    const handleStatusChange = async (id, checked) => {
        ...
        try {
            ...
            // update local state
            setPolicies((prev) => prev.map((item) => (item.id === id ? { ...item, is_active: checked } : item)));
    ...
    const columns = [
        ...
        {
            field: "effect_default", headerName: "Effect", width: 110,
            renderCell: (p: any) => (
                <Chip
                    size="small"
        ...
        {
            field: "policy_type", headerName: "Policy Type", width: 130,
            renderCell: (p: any) => (
                <Typography variant="body2" sx={{ textTransform: "capitalize" }}>{p.row.policy_type}</Typography>
            ),
        },
        {
            field: "resources", headerName: "Resources", flex: 1, minWidth: 180,
            renderCell: (p: any) => (
        ...
        {
            field: "actions", headerName: "Actions", flex: 1, minWidth: 180,
            renderCell: (p: any) => (
                <Stack direction="row" spacing={0.5} sx={{ flexWrap: "wrap", gap: 0.5 }}>
                    {p.row.actions.slice(0, 3).map((a: string) => (
                        <Chip key={a} size="small" label={a} />
                    ))}
                    {p.row.actions.length > 3 && (
                        <Chip size="small" label={`+${p.row.actions.length - 3}`} />
                    )}
                </Stack>
            ),
        },
        { field: "priority", headerName: "Priority", width: 100 },
        {
            ...
            renderCell: (p: any) => (<>
                {!p.row.is_active ? <Badge color="success">Active</Badge> : <Badge color="error">Inactive</Badge>   }
                <Switch
                    color="success"
        ...
        {
            field: "rowActions", headerName: "Actions", width: 140, sortable: false, filterable: false,
            renderCell: (p: any) => (
                <Stack direction="row" spacing={0.5}>
                    <Tooltip title="Edit">
                        <IconButton size="small" onClick={() => openEdit(p.row)}><Pencil size={15} /></IconButton>
                    </Tooltip>
        ...
        <Box>
                    ...
                    {editing && (
                        <Grid container spacing={2}>
                            <Grid item xs={12} md={8}>
                                <TextField
                                    fullWidth required label="Policy name" value={editing.name}
                                    onChange={(e) => setEditing({ ...editing, name: e.target.value })}
                                />
                            </Grid>
                            <Grid item xs={6} md={4}>
                                <TextField
                                    fullWidth select label="Effect" value={editing.effect}
                                    onChange={(e) => setEditing({ ...editing, effect: e.target.value as PolicyEffect })}
                                >
                                    <MenuItem value="allow">Allow</MenuItem>
                                    <MenuItem value="deny">Deny</MenuItem>
                                </TextField>
                            ...
                            <Grid item xs={12}>
                                <TextField
                                    fullWidth multiline rows={2} label="Description" value={editing.description}
                                    onChange={(e) => setEditing({ ...editing, description: e.target.value })}
                                />
                            </Grid>
                            <Grid item xs={6} md={4}>
                                <TextField
                                    fullWidth select label="Scope" value={editing.scope}
                                    onChange={(e) => setEditing({ ...editing, scope: e.target.value as Policy["scope"] })}
                                >
                            ...
                            <Grid item xs={6} md={4}>
                                <TextField
                                    fullWidth select label="Status" value={editing.status}
                            ...
                            <Grid item xs={12} md={4}>
                                <Stack sx={{ px: 1 }}>
                                    <Typography variant="caption" color="text.secondary">Priority ({editing.priority})</Typography>
                                    <Slider
                            ...
                            <Grid item xs={12}><Divider><Typography variant="caption">Targets</Typography></Divider></Grid>

                            <Grid item xs={12} md={6}>
                                <Autocomplete
                                    multiple freeSolo options={RESOURCE_OPTIONS}
                                    ...
                                    renderInput={(params) => <TextField {...params} label="Resources" placeholder="Add resource" />}
                                />
                            </Grid>
                            <Grid item xs={12} md={6}>
                                <Autocomplete
                                    multiple freeSolo options={ACTION_OPTIONS}
                                    value={editing.actions}
                                    onChange={(_, v) => setEditing({ ...editing, actions: v })}
                                    renderTags={(value, getTagProps) =>
                                        value.map((opt, i) => <Chip size="small" label={opt} {...getTagProps({ index: i })} key={opt} />)
                                    }
                                    renderInput={(params) => <TextField {...params} label="Actions" placeholder="Add action" />}
                                />
                            </Grid>
                            <Grid item xs={12}>
                                <Autocomplete
                                    multiple freeSolo options={SUBJECT_OPTIONS}
                                    value={editing.subjects}
                                    onChange={(_, v) => setEditing({ ...editing, subjects: v })}
                                    renderTags={(value, getTagProps) =>
                                        value.map((opt, i) => <Chip size="small" label={opt} {...getTagProps({ index: i })} key={opt} />)
                                    }
                                    renderInput={(params) => <TextField {...params} label="Subjects (roles or users)" placeholder="role:admin, user:..." />}
                                />
                            </Grid>
                            <Grid item xs={12}>
                                <TextField
                                    fullWidth multiline rows={4} label="Conditions (JSON)"
            ...
            <Dialog open={!!confirmDelete} onClose={() => setConfirmDelete(null)}>
                ...
                <DialogActions>
                    ...
                    <Button color="error" variant="contained" onClick={() => confirmDelete && remove(confirmDelete)}>
                        Delete
                    </Button>
                </DialogActions>
            </Dialog>
            ...
            >
                {toast ? <Alert severity={toast.type} variant="filled">{toast.msg}</Alert> : undefined}
            </Snackbar>
        </Box>
    );
}